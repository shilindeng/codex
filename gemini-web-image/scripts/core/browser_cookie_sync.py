...
from .common import ensure_dir, write_cookie_payload
...
def crypto_status() -> dict[str, Any]:
    return {
        "available": AESGCM is not None,
        "error": CRYPTOGRAPHY_IMPORT_ERROR,
    }
...
def _load_master_key(local_state_path: Path) -> bytes:
    if AESGCM is None:
        raise RuntimeError(f"cryptography not available: {CRYPTOGRAPHY_IMPORT_ERROR}")
...
def _copy_cookie_db(path: Path) -> Path:
    handle = tempfile.NamedTemporaryFile(prefix="gemini-web-image-cookie-", suffix=".sqlite", delete=False)
    handle.close()
    ...
    else:  # pragma: no cover - Windows is the target path here
...
def import_google_cookies(target_path: Path) -> dict[str, Any]:
    if os.name != "nt":
        return {"ok": False, "error": "cookie import is only supported on Windows", "target_path": str(target_path)}
    if AESGCM is None:
        return {"ok": False, "error": f"cryptography not available: {CRYPTOGRAPHY_IMPORT_ERROR}", "target_path": str(target_path)}
    ...
    write_cookie_payload(target_path, best_payload)
    return {
        "ok": True,
        "source": best_source,
