...
- 先判断这篇该怎么写，再动笔。
- 先做诊断，再决定要不要继续写；主题、产品、证据、开头素材不够时，优先打回上游，不要硬凑成稿。
- 先加载 `account-strategy.json`、近期语料和作者记忆，再做标题、大纲、正文、配图和验收。
- 显式建立 `viral_blueprint` 和 `editorial_blueprint`，不要靠临时补禁用词赌结果。
- 标题先判断触发器，开头先检查“独立性 + 悬念 + 可信度”，不要默认套万能模板。
- 全文默认按三层结构交付：`钩子 -> 认知增量 -> takeaway`，缺任一层都不算完成。
- 去 AI 味不是删几个词，而是逐条排查 AI 指纹：替读者发言、翻译腔、深刻腔、祝福腔、转化页开头，都要优先打掉。
...
- 运行时默认不能依赖某一台机器的固定盘符和本地 jobs 目录，优先走显式参数、环境变量和相对路径。
...
- 中段至少有一处案例、数据或事实托底。
- 全文至少有一处反方、误判或适用边界。
- 最后 15%~20% 正文里必须落下可收藏、可复用、可转发的 takeaway。
- 至少保留 1~2 段真正展开的分析段。
- 正文和首屏不能泄漏内部提示语或写作说明。
...
- 不要出现“大家好，我是…… / 今天想聊……”这种死开头。
- 不要写成“如果你也很焦虑……这篇文章会告诉你答案”的转化页开场。
- 不要反复复用“为什么大多数人……”“真正危险的不是……而是……”“先想清 3 件事”这类旧模板。
- 不要替读者预设一句蠢话再纠正，比如“你可能会觉得……但其实……”
...
- `codex` / Codex App 默认按 `gpt-image-2` 适配：所有图片都必须带可读中文短字，封面图写 1~2 行短标题，正文图写 1 个中文关键词或短判断，结构图最多 2~3 个短标签；如果实际成图无字、缺字、乱码或字太小，必须重生，不能直接登记。
- `codex` 正文图默认优先公众号运营语境：封面要促点击，正文图要有场景/物件/人物压力/海报感，结尾图要适合收藏或转发；不要把整篇做成架构图、流程图、仪表板或 UI 线框。
