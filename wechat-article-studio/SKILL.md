...
- 若用户未显式指定图片参数，默认按文章内容、分类与章节语义自动选择图片主题与风格；显式参数始终优先于自动决策。
- 默认图片密度使用 `balanced`；除非用户明确要求更少/更多图片，否则保持读者友好的图文密度。
- `--image-style-mode` 不再默认固定为某一种风格；当用户未指定时，由文章内容自动决定 `uniform` 或 `mixed-by-type`。
...
- 默认正文模式为 `tech-balanced`：先保证技术可信、结构清晰、术语准确，再追求传播性。
- 宿主或 provider 生成正文时，不要输出顶层 `# 标题`；标题只保留在 frontmatter / manifest。
- 命令、包名、环境变量、路径、接口名、模型名、代码符号、英文专有名词优先使用反引号渲染。
...
python {SKILL_DIR}/scripts/studio.py hosted-run \
  --workspace <job-dir> \
  --topic "<主题>" \
  --content-mode tech-balanced \
  --wechat-header-mode drop-title \
  --to render
```
...
- 若未显式设置上述参数，系统会根据文章内容自动选择 `preset/style_mode/layout_family`，并把原因写入 `manifest.json` 与 `image-plan.json`。
...
- `--wechat-header-mode`：公众号 HTML 头部策略：`keep|drop-title|drop-title-summary`（默认 `drop-title`，只保留摘要卡，不输出标题）。
