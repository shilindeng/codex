...
- `score-report.json` 必须过线，且“可信度与检索支撑”达到最低阈值
...
- 无主题启动链路默认启用 `--image-style-mode mixed-by-type`：按图片类型混合风格（封面/信息图/正文插图），但保持整篇配色与母题一致；可用 `--image-style-mode uniform` 显式切回统一风格。
...
- 纯 CLI 场景下，`run / research / titles / outline / write / review` 缺少文本 API 配置会直接失败，不再静默产出 placeholder 稿。
