...
"""
    ...
    def test_codex_plan_prompts_are_wechat_editorial_with_readable_chinese_text(self):
        article = """---
title: OpenAI 把 Agent 放进团队工作流
summary: 这不是又一个功能更新，而是团队协作里责任边界和验收方式开始变化。
---

## 第一层变化：入口被放到真实工作里

一个产品经理打开团队工作区，看到的不是炫技演示，而是任务、文件、讨论和 Agent 混在同一个协作现场。

## 第二层变化：责任边界会先变紧

真正麻烦的地方不是工具多了，而是出了错以后谁来验收、谁来回滚、谁来承担风险。

## 最后的判断

这件事适合做成一张能转发的判断图：先补责任边界，再谈效率提升。
"""
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            (workspace / "article.md").write_text(article, encoding="utf-8")
            legacy.cmd_plan_images(make_args(workspace, provider="codex", inline_count=3))
            plan = json.loads((workspace / "image-plan.json").read_text(encoding="utf-8"))
            strategy = json.loads((workspace / "image-strategy.json").read_text(encoding="utf-8"))
            middle_items = [item for item in plan["items"] if item.get("insert_strategy") == "section_middle"]
            self.assertNotIn(plan["image_controls"].get("layout_family"), {"process", "dashboard", "hierarchy"})
            self.assertEqual(plan["image_controls"]["text_policy_overrides"]["正文插图"], "short-zh")
            self.assertIn(strategy.get("visual_route"), {"cover-poster", "scene-metaphor", "reader-emotion", "share-card", "contrast-poster"})
            self.assertTrue(all(item.get("required_text") for item in plan["items"]))
            inline_labels = [label for item in middle_items for label in (item.get("required_text") or [])]
            self.assertTrue(any("责任" in label or "验收" in label for label in inline_labels))
            self.assertLessEqual(sum(1 for item in middle_items if item["type"] == "流程图"), 1)
            prompt_text = "\n".join(
                (workspace / item["prompt_path"]).read_text(encoding="utf-8")
                for item in json.loads((workspace / "image-outline.json").read_text(encoding="utf-8"))["items"]
            )
            self.assertIn("gpt-image-2", prompt_text)
            self.assertIn("Chinese text must be visible", prompt_text)
            self.assertIn("Do not make a generic system architecture diagram", prompt_text)
...
2. 第二条"""
    ...
    def test_comparison_prompt_allows_short_chinese_labels(self):
        item = {
            "id": "inline-01",
            "type": "对比图",
        ...
        self.assertIn("Text policy: short-zh", prompt)
        self.assertIn("Preferred label language: zh-CN", prompt)
        self.assertIn("Allowed labels:", prompt)
        self.assertIn("Simplified Chinese labels", prompt)
        self.assertNotIn("Do not include any readable Chinese or English words", prompt)

    def test_cover_prompt_keeps_no_readable_text_policy(self):
        item = {
            "id": "cover-01",
            "type": "灏侀潰鍥?",
            "section_heading": "娴嬭瘯灏侀潰",
            "section_excerpt": "杩欐槸涓€绡囬渶瑕佺涓€鐪艰蹇嗙偣鐨勬枃绔犮€?",
            "layout_variant_label": "涓績涓昏瑙?",
            "layout_variant_instruction": "Use one dominant hero object.",
            "visual_theme": "娴嬭瘯涓婚",
            "visual_style": "娴嬭瘯椋庢牸",
            "visual_mood": "娴嬭瘯姘涘洿",
            "visual_brief": "Keep it simple.",
        }
        controls = {"preset": "fresh", "preset_label": "娓呮柊鏉傚織", "style_mode": "uniform"}
        prompt = legacy.compose_prompt("娴嬭瘯鏍囬", "娴嬭瘯鎽樿", controls, item, "娴嬭瘯璇昏€?")
        self.assertIn("Text policy: none", prompt)
        self.assertIn("Do not include any readable Chinese or English text", prompt)

    def test_image_section_focus_prefers_heading_over_long_excerpt(self):
        item = {
            "section_heading": "鏍稿績瀵规瘮",
            "section_excerpt": "杩欐槸涓€娈垫瘮杈冮暱鐨勮鏄庯紝鏈潵涓嶅簲璇ヨ鏁翠綋鎷煎埌 focus 閲屻€?",
            "anchor_block_excerpt": "鍐嶉暱涓€鐐圭殑娈佃惤涔熶笉搴旇鍘嬭繘 focus 銆?",
        }
        focus = legacy.image_section_focus(item, 64)
        self.assertEqual(focus, "鏍稿績瀵规瘮")

    def test_gemini_web_prompt_variant_keeps_short_chinese_policy(self):
        prompt = "\n".join(
            [
                ...
                "Style: 娴嬭瘯椋庢牸",
                "Mood: 娴嬭瘯姘涘洿",
                "Visual brief: Keep it simple.",
                "Section focus: 鏍稿績瀵规瘮",
                "Section excerpt: 杩欓噷闇€瑕佷袱涓煭鏍囩甯姪鐪嬫噦瀵规瘮鍏崇郴銆?",
                "Layout variant: 宸﹀彸鍒嗘爮",
                "Composition rule: Use split panels.",
                "Text budget: up to 4 very short contrast labels",
                "Text policy: short-zh",
                "Preferred label language: zh-CN",
                "Allowed labels: 鏂规A / 鏂规B",
            ]
        )
        variants = dict(legacy.gemini_web_prompt_variants(prompt))
        self.assertIn("compact-scene", variants)
        self.assertIn("minimal-scene", variants)
        self.assertIn("Simplified Chinese labels", variants["compact-scene"])
        self.assertIn("鏂规A / 鏂规B", variants["compact-scene"])
        self.assertNotIn("No text in image", variants["compact-scene"])
