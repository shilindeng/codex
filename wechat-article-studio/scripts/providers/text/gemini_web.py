...
class GeminiWebTextProvider(TextProvider):
    ...
    def generate_article(self, context: dict[str, Any]) -> ProviderResult:
        prompt = (
            ...
            "19. 前 300~400 字内必须同时出现主判断和一个具体场景或动作，第一个二级标题前最多 4 段。"
            "20. 正文必须写清至少 1 段现实代价：谁在付、付在哪里、会多出什么后果。"
            "21. 正文必须留出至少 1 个自然的讨论入口或分歧点，但不要把它写成最后一行的硬互动。"
            f"\n输入：{json.dumps(context, ensure_ascii=False)}"
        )
    ...
    def revise_article(self, context: dict[str, Any]) -> ProviderResult:
        prompt = (
            ...
            "11. 如果输入给了 humanizerai_detection 或 humanizerai_humanized_body，把它当外部去味参考稿，只借它的去味方向，不要照抄，也不要丢掉原文事实、Markdown 结构和账号风格。"
            "12. 额外避开这几类 AI 腔：反复写“不是X，而是Y”“问题不在X，而在Y”，反复用“换句话说/更重要的是/真正的问题是”先铺垫再说重点，以及让“数据/趋势/市场/AI/系统”替人做动作。"
            "13. 如果首屏超过 4 段，优先压缩首屏，把主判断和具体场景前移。"
            "14. 如果正文没有现实代价，就优先补一段“谁在付、付在哪里、会多出什么后果”。"
            "15. 如果正文没有讨论入口，就在中段补一个自然分歧点，不要在最后一行硬加提问。"
            f"\n输入：{json.dumps(context, ensure_ascii=False)}"
        )
        text = self._run_prompt(prompt, expect_json=False)
        return ProviderResult(payload=_strip_fences(text).strip() + "\n", provider=self.provider_name, model=self.model)
