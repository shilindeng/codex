...
class OpenAICompatibleTextProvider(TextProvider):
    ...
    def generate_research_pack(self, context: dict[str, Any]) -> ProviderResult:
        ...
        prompt = [
            {
                "role": "system",
                "content": "你是微信公众号选题研究助手。只输出 JSON，字段为 topic, angle, audience, sources, evidence_items, information_gaps, forbidden_claims。若输入带有 account_strategy，优先服从账号定位、目标读者和最小证据要求。",
            },
        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "如果输入包含 account_strategy，要先服从账号定位、目标读者和首要目标，不要回到通用公众号腔。"
        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "如果输入带有 account_strategy，正文必须先服务账号定位、目标读者和首要目标；当前阶段优先打开率和读完率，不要硬追虚高互动。"
        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "如果输入带有 account_strategy，改稿时先服务账号定位、目标读者和首要目标，优先修标题、首屏、证据和阅读推进。"
