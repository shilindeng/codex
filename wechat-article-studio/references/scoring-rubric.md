...
- 正式合格线统一为 `total_score >= 88`，即使旧报告写着 `passed=true`，低于 88 也不能算通过。
- 标题、首屏、证据、传播点、排版、图片、草稿回读都必须同时通过。
- `fact-source-map.json` 不通过时，说明关键事实没有来源对应，必须补来源或删改事实。
- `section-quality-map.json` 不通过时，说明核心章节缺少观点、事实或分析推进，必须返工正文。
- `image-asset-audit.json` 不通过时，说明图片文件、尺寸、占位图或重复图有问题，不能进入真合格。
- 强制发布只允许完成草稿箱交付，不改变质量分数，也不能计入真合格成品。
